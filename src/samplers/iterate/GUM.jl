function codegen(::Type{Val{:iterate}}, ::Type{GUM}, job::BasicMCJob)
  result::Expr
  burninbody = []
  ambody = []
  ...
  if vform != Multivariate
    error("Only multivariate parameter states allowed in GUM code generation")
  end
  ...
  push!(ambody, :(_job.sstate.randnsample = randn(_job.pstate.size)))

  push!(
    ambody,
    :(
      if _job.sstate.pastupdatetensor
        _job.sstate.cholinvtensor = _job.sstate.sqrttunestep*chol(_job.sstate.oldinvtensor, Val{:L})
      end
    )
  )

  push!(ambody, :(_job.sstate.pstate.value = _job.pstate.value+_job.sstate.cholinvtensor*_job.sstate.randnsample))
  ...
  push!(ambody, :(_job.sstate.η = min(1, _job.pstate.size*_job.sstate.count^(-_job.sampler.γ))))
  ...
  push!(ambody, :(_job.sstate.SST = _job.sstate.cholinvtensor*(eye(_job.pstate.size)+_job.sstate.SST)*_job.sstate.cholinvtensor'))

  push!(ambody, :(_job.sstate.cholinvtensor = chol(_job.sstate.SST, Val{:L})))
